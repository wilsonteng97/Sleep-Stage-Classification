...
class DataExtractor():
    ...
    def __init__(self, x, y, config=Config(), collated_freq_bands_path=None):
        ...
        if False: # os.path.exists(collated_freq_bands_path):
            ...
            # self.A2, self.D2, self.D1 = pywt.wavedec(self.x, 'db4', level=2)
            ...
            self.freq_band_dict = {
                ...
                # "D2": self.D2,
                # "D1": self.D1,
                "xFFT": np.absolute(fft(self.x)),
                "deltaFFT": np.absolute(fft(self.delta)),
                "thetaFFT": np.absolute(fft(self.theta)), 
                "alphaFFT": np.absolute(fft(self.alpha)), 
                "sigmaFFT": np.absolute(fft(self.sigma)), 
                "beta1FFT": np.absolute(fft(self.beta1)), 
                "beta2FFT": np.absolute(fft(self.beta2)), 
                "gamma1FFT": np.absolute(fft(self.gamma1)), 
                "gamma2FFT": np.absolute(fft(self.gamma2)),
            }
            print("self.freq_band_dict created.")
            # print("self.freq_band_dict['XFFT']: ", self.freq_band_dict["xFFT"])
            # print("self.freq_band_dict['deltaFFT']: ", self.freq_band_dict["deltaFFT"])
            # print("self.freq_band_dict['thetaFFT']: ", self.freq_band_dict["thetaFFT"])
            # print("self.freq_band_dict['alphaFFT']: ", self.freq_band_dict["alphaFFT"])
    ...
    def generateDF(self):
        ...
        df['hjorth_activity'], df['hjorth_morbidity'], df['hjorth_complexity'] = self.calculateHjorth(self.x)
        ...
        for i, band_name in enumerate(self.freq_band_dict.keys()):
            ...
            df[band_name + 'Hjorth_activity'], df[band_name + 'Hjorth_morbidity'], df[band_name + 'Hjorth_complexity'] = self.calculateHjorth(data)
